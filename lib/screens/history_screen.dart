...
class _HistoryScreenState extends State<HistoryScreen> with SingleTickerProviderStateMixin {
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('${item.treatment} | Block ${item.block}', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: AppTheme.textDark)),
  ...
  Widget _buildGridMetric(String label, String value, {String? subtext}) {
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 4),
        Text(value, style: const TextStyle(fontSize: 16, color: AppTheme.textDark, fontWeight: FontWeight.bold)),
        if (subtext != null)
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Data & Export', style: TextStyle(color: AppTheme.textDark, fontWeight: FontWeight.bold)),
      ...
      body: TabBarView(
        controller: _tabController,
        children: [
          _buildList(storage.productionLogs, true),
          _buildList(storage.eggLogs, false),
        ],
      ),
  ...
  Widget _buildList(List<dynamic> logs, bool isProd) {
    if (logs.isEmpty) {
      return const Center(child: Text('No logs found.', style: TextStyle(color: AppTheme.textMuted)));
    }
