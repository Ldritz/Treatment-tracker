class EggLog {
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'w3': w3,
      'width': width,
      'albumen_height': albumenHeight,
      'shell_weight': shellWeight,
      'yolk_weight': yolkWeight,
      'haugh_unit': haughUnit,
  ...
  factory EggLog.fromJson(Map<String, dynamic> json) {
    return EggLog(
      ...
      albumenHeight: (json['albumen_height'] ?? json['albumenHeight'] as num?)?.toDouble() ?? 0.0,
      shellWeight: (json['shell_weight'] ?? json['shellWeight'] as num?)?.toDouble() ?? 0.0,
      yolkWeight: (json['yolk_weight'] ?? json['yolkWeight'] as num?)?.toDouble() ?? 0.0,
      haughUnit: (json['haugh_unit'] ?? json['haughUnit'] as num?)?.toDouble() ?? 0.0,
      shapeIndex: (json['shape_index'] ?? json['shapeIndex'] as num?)?.toDouble() ?? 0.0,
      yolkPct: (json['yolk_pct'] ?? json['yolkPct'] as num?)?.toDouble() ?? 0.0,
      isDeleted: (json['is_deleted'] ?? json['isDeleted']) as bool? ?? false,
      isSynced: (json['is_synced'] ?? json['isSynced']) as bool? ?? false,
      lastModified: (json['last_modified'] ?? json['lastModified'] ?? json['timestamp']) as String,
    );
  }
}
